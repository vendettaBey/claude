  ...
  return (
    <SelectPrimitive.Root value={value} onValueChange={onValueChange} name={name}>
      <SelectPrimitive.Trigger
        ...
        className={cn(
          'flex h-12 w-full items-center justify-between gap-3 rounded-xl border px-4 text-left text-[0.95rem]',
      ...
      >
        ...
        <SelectPrimitive.Icon asChild>
          <ChevronDown className="size-4 shrink-0 text-fg-dim" aria-hidden="true" />
      ...
      <SelectPrimitive.Portal>
        <SelectPrimitive.Content
          ...
          className={cn(
            ...
            'border border-white/12 bg-ink-850/98 shadow-[0_24px_60px_-24px_rgba(0,0,0,0.95)] backdrop-blur-xl',
          )}
        >
          <SelectPrimitive.Viewport className="p-1.5">
            {options.map((option) => (
              <SelectPrimitive.Item
                ...
                value={option.value}
                className={cn(
                  'relative flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2.5 pr-9 text-sm text-fg-muted',
                  'transition-colors outline-none select-none',
                  'data-[highlighted]:bg-white/[0.07] data-[highlighted]:text-fg',
